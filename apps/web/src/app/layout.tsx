  ...
  return (
    <html lang="en">
      <head>
        ...
        <script dangerouslySetInnerHTML={{ __html: `try{var s=localStorage.getItem('keystone-theme');var dk=s==='dark';var tc=dk?'#F5E6D3':'#2C1810';if(dk){document.documentElement.classList.add('dark');}var m=document.querySelector('meta[name="theme-color"]');if(m)m.setAttribute('content',tc);window.__ksThemeColor=tc;window.__ksFavicon=dk?'/favicon-dark.svg':'/favicon.svg';new MutationObserver(function(){var el=document.querySelector('meta[name="theme-color"]');if(el&&el.getAttribute('content')!==window.__ksThemeColor){el.setAttribute('content',window.__ksThemeColor);el.removeAttribute('media');}var ic=document.querySelector('link[rel="icon"][type="image/svg+xml"]');if(ic&&ic.getAttribute('href')!==window.__ksFavicon){ic.setAttribute('href',window.__ksFavicon);}}).observe(document.head,{childList:true,subtree:true,attributes:true});}catch(e){}` }} />
        <link
