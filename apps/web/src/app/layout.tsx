...
export default function RootLayout({
  ...
  return (
    <html lang="en">
      <head>
        <script dangerouslySetInnerHTML={{ __html: `try{var s=localStorage.getItem('keystone-theme');var d=window.matchMedia('(prefers-color-scheme:dark)').matches;if(s==='dark'||(s!=='light'&&d)){var r=document.documentElement;r.classList.add('dark');r.style.setProperty('--color-background','#1A1412');r.style.setProperty('--color-foreground','#E8DDD0');r.style.setProperty('--color-surface','#231E1A');r.style.setProperty('--color-surface-alt','#2C2520');r.style.setProperty('--color-border','#3D342C');r.style.setProperty('--color-muted','#9A8E82');r.style.setProperty('--color-earth','#E8DDD0');r.style.setProperty('--color-warm','#2C2520');document.body.style.backgroundColor='#1A1412';document.body.style.color='#E8DDD0';}}catch(e){}` }} />
        <link
