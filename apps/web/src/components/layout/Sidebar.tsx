...
/*
 * Sidebar color tokens — uses hardcoded hex for reliability.
 * Dark mode inversion is handled by wrapping the entire sidebar
 * in a container with CSS variables that flip in dark mode.
 */
const C = {
  bg: "bg-[var(--sidebar-bg)]",
  bgLight: "bg-[var(--sidebar-bg-hover)]",
  text: "text-[var(--sidebar-text)]",
  textBright: "text-[var(--sidebar-text-bright)]",
  border: "border-[var(--sidebar-text)]",
} as const;
...
export function Sidebar({
      ...
      <div key={item.id} className="relative group">
        <button
          ...
          className={`
            ...
            ${
              isActive
                ? "border-l-emerald-500 rounded-r-lg bg-emerald-500/8 text-[var(--sidebar-text-bright)] opacity-100"
                : "border-l-transparent text-[var(--sidebar-text)] opacity-50 hover:opacity-80 hover:bg-[var(--sidebar-text)]/5 hover:border-l-[#D4A574]/30 hover:border-l-[2px]"
            }
          `}
        ...
        {collapsed && (
          <div className={`absolute left-full top-1/2 -translate-y-1/2 ml-2 px-2.5 py-1 rounded-md ${C.bgLight} text-[var(--sidebar-text-bright)] text-[11px] whitespace-nowrap opacity-0 pointer-events-none group-hover:opacity-100 transition-opacity duration-150 z-50 shadow-md`}>
            {translatedLabel}
          </div>
        )}
      </div>
  ...
  return (
    <>
      ...
      <aside
        className={`
          fixed top-0 left-0 bottom-0 ${sidebarWidth} ${C.bg} text-[var(--sidebar-text)] z-40
      ...
      >
        ...
        <div className={`px-5 pt-5 pb-4 border-b border-[#D4A574]/10 flex items-center ${collapsed ? "justify-center" : "justify-between"}`}>
          {collapsed ? (
            <Link href="/dashboard" className="flex items-center justify-center">
              <KeystoneIcon size={22} className="text-[var(--sidebar-text)]" />
            </Link>
          ) : (
            <>
              <Link href="/dashboard" className="flex items-center gap-2.5">
                <KeystoneIcon size={22} className="text-[var(--sidebar-text)]" />
                <span className="text-[15px] font-semibold text-[var(--sidebar-text-bright)] tracking-tight">
                  Keystone
                </span>
              </Link>
              <button
                onClick={onClose}
                className="lg:hidden p-1 rounded text-[var(--sidebar-text)]/50 hover:text-[var(--sidebar-text)]/80 transition-colors"
              >
                <X size={18} />
              </button>
            </>
        ...
        <nav className="py-3 shrink-0">
          {!collapsed && (
            <p className="pl-4 pr-3 mb-1.5 text-[9px] uppercase tracking-[2px] text-[var(--sidebar-text)]/30 font-medium">
        ...
        {projectName && (
          <nav className="py-3 flex-1 min-h-0 overflow-y-auto overflow-x-hidden sidebar-scroll">
            {!collapsed && (
              <p className="pl-4 pr-3 mb-1.5 text-[9px] uppercase tracking-[2px] text-[var(--sidebar-text)]/30 font-medium truncate" title={projectName}>
                {projectName}
              </p>
            )}
                ...
                <div key={group.label}>
                  {!collapsed && (
                    <button
                      ...
                      className="w-full pl-4 pr-3 mt-3 mb-1 flex items-center justify-between text-[9px] uppercase tracking-[2px] text-[var(--sidebar-text)]/30 font-medium hover:text-[var(--sidebar-text)]/50 transition-colors"
                    >
        ...
        {/* Language indicator */}
        {showLangBadge && (
          <div className={`px-5 py-2 flex items-center gap-2 text-[var(--sidebar-text)]/40 ${collapsed ? "justify-center" : ""}`}>
            <Globe size={12} />
            {!collapsed && (
        ...
        <div className="hidden lg:flex px-3 py-2 justify-center">
          <button
            ...
            className="p-1.5 rounded text-[var(--sidebar-text)]/30 hover:text-[var(--sidebar-text)]/60 hover:bg-[var(--sidebar-text)]/5 transition-all duration-150"
        ...
        >
          ...
          <button
            ...
            className={`
              ...
              ${activeSection === "settings"
                ? "opacity-100 bg-emerald-500/8 border-l-emerald-500 text-[var(--sidebar-text-bright)]"
                : "opacity-50 border-l-transparent text-[var(--sidebar-text)] hover:opacity-80 hover:bg-[var(--sidebar-text)]/5 hover:border-l-[2px] hover:border-l-[#D4A574]/30"
              }
          ...
          <div className={`px-3 py-3 flex items-center ${collapsed ? "justify-center" : "gap-2.5 px-5"}`}>
            <div className="w-8 h-8 rounded-full bg-[var(--sidebar-text)]/15 flex items-center justify-center text-[11px] font-semibold text-[var(--sidebar-text)] flex-shrink-0">
              {initials}
            </div>
            {!collapsed && (
              <>
                <div className="flex-1 min-w-0">
                  <p className="text-[12px] text-[var(--sidebar-text-bright)] truncate">{userName}</p>
                  <p className="text-[10px] text-[var(--sidebar-text)]/35">{planLabel}</p>
                </div>
                <ThemeToggle />
                {onSignOut && (
                  <button
                    onClick={onSignOut}
                    className="p-1.5 rounded text-[var(--sidebar-text)]/40 hover:text-[var(--sidebar-text)]/80 transition-colors"
                    title="Sign out"
                  >
                    <LogOut size={14} />
                  </button>
